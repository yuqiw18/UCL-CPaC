...
imageSequence = GlobalFlickerReduction(first, imageSequence, sceneClipPoints);
imageSequence = BlotchCorrection(first, imageSequence, sceneCutFrames, sceneClipPoints);
imageSequence = VerticalArtifactReduction(first, imageSequence, sceneCutFrames, sceneClipPoints);
imageSequence = CameraShakeCalibration(first, imageSequence, sceneClipPoints);
...
function [sceneCutFrames, sceneClipPoints] = SceneCutDetection(first, last, imageSequence)
    ...
    for i = 2 : sequenceLength-1
        % Find the transitional difference
...
function imageSequence = GlobalFlickerReduction(first, imageSequence, sceneClipPoints)
    ...
    for p = 1 : 2 : sceneClipPointsCount-1
        for i = sceneClipPoints(p):sceneClipPoints(p+1)    
            ...
            % Find neighbouring frames for averaging
...
function imageSequence = BlotchCorrection(first, imageSequence, sceneCutFrames, sceneClipPoints)
    ...
    sceneMotionThreshold = 0.3;
    blotchThreshold = 0.12;
    
    expansionFilter = fspecial('average', 40);
    dilateStructure = strel('disk', 32, 4);
    ...
    motionMaskSequence = imfilter(motionMaskSequence, expansionFilter);
    motionMaskSequence(motionMaskSequence<sceneMotionThreshold)=0;
    motionMaskSequence(motionMaskSequence>sceneMotionThreshold)=1;
    motionMaskSequence = imdilate(motionMaskSequence, dilateStructure);   
    motionMaskSequence = imfill(imbinarize(motionMaskSequence),'holes'); 
...
function imageSequence = VerticalArtifactReduction(first, imageSequence, sceneCutFrames, sceneClipPoints)
...
function imageSequence = CameraShakeCalibration(first, imageSequence, sceneClipPoints)
